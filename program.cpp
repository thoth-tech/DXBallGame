...
bitmap get_powerup_bitmap(powerups kind)
{
    ...
    case SCORE_MULTIPLY:
        if (game_data.score_multiplier < 5)
            return bitmap_named("dropped_multiplier_" + to_string(game_data.score_multiplier + 1));
        else
            return bitmap_named("dropped_multiplier_5"); // prevent missing bitmap errors
...
void end_game(bool successful)
{
    ...
    if (!game_data.initials_entered && high_score_position() != 10) // draw initial entry screen if current score is a new high score, until initials are submitted
    {
        ...
        draw_text("Button 1 to submit", COLOR_BLACK, font_named("default"), 30, 240, 280); //x was 260

        // change initial
        if (key_typed(UP))
        {
            if (*initials_entry.current_character == 26)
                *initials_entry.current_character = 0; // loop back to '-' if going over maximum
            else
        ...
        else if (key_typed(DOWN))
        {
            if (*initials_entry.current_character == 0)
                *initials_entry.current_character = 26; // loop back to 'z' if going under minimum
            else
                *initials_entry.current_character = *initials_entry.current_character - 1;
        }
    ...
    else // draw scoreboard
    {
        ...
        draw_text("Press Start to play again", COLOR_BLACK, font_named("default"), 18, 260, 540); //x was 275
    }
}
...
{
    if (game_data.current_balls[i].up)
        game_data.current_balls[i].y -= BALL_SPEED;
    else
        game_data.current_balls[i].y += BALL_SPEED;
    if (game_data.current_balls[i].right)
        game_data.current_balls[i].x += BALL_SPEED;
    else
        game_data.current_balls[i].x -= BALL_SPEED;
}
...
void update_powerup_drops(int i)
{
    ...
    if ((PADDLE_Y <= game_data.current_powerups[i].y + BALL_RADIUS and game_data.current_powerups[i].y + BALL_RADIUS <= PADDLE_Y + (BALL_SPEED / 2)) and (game_data.current_powerups[i].x + BALL_RADIUS >= game_data.paddle_x and game_data.current_powerups[i].x - BALL_RADIUS <= game_data.paddle_x + PADDLE_LENGTH))
    {
        ...
        if (game_data.current_powerups[i].kind == MULTI_BALL)
            game_data.current_balls.push_back(create_ball(game_data.current_powerups[i].x, PADDLE_Y + 10, true, direction)); // create ball above the paddle with a random horizontal direction
...
int main()
{
    ...
    current_window_toggle_fullscreen(); // toggle fullscreen
    ...
    while (!key_down(ESCAPE_KEY))
    {
        ...
        else
        {
            if (game_data.timer > 0)
                game_data.timer -= (1.0 / 60.0); // count down 1/60 seconds every frame if the timer is in use
            else if (game_data.timer < 0)
                game_data.timer = 0;
            else
                game_data.score_multiplier = 1;

            if (game_data.extraBallTimer > 0)
                game_data.extraBallTimer -= (1.0 / 60.0); // count down extra ball timer
            ...
            // Player controls
            if (key_down(LEFT) and game_data.paddle_x > 10)
                game_data.paddle_x -= PADDLE_SPEED; // moving left
            if (key_down(RIGHT) and game_data.paddle_x < screen_width() - PADDLE_LENGTH - 10)
                game_data.paddle_x += PADDLE_SPEED; // moving right
